...
enum EntityType { PLATFORM, PLAYER, ENEMY, BUBBLE, HOOK };
enum AIType { WALKER, GUARD, DROPPER };
...
public:
    ...
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[4][4], float animation_time,
        ...
        int animation_rows, float width, float height, EntityType EntityType);      // PLAYER CONSTRUCT
    ...
    void shoot_left() { bubble_direction = 4; face_left(); }
    void shoot_right() { bubble_direction = 2; face_right(); }
    ...
    bool const get_is_active() { return m_is_active; }
    ...
    void const decrease_lives() { m_lives--; }
    void const set_is_hit_false() { is_hit = false; }
    void const set_is_hit_true() { is_hit = true; }
